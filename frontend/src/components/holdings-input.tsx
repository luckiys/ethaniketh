...
function CryptoSelect({ value, onChange, disabled, prices }: CryptoSelectProps) {
  ...
  // Fetch prices for visible dropdown items (debounced 200ms)
  useEffect(() => {
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {open && (
        <div className="absolute left-0 top-full z-50 mt-1 w-60 rounded-md border border-zinc-700 bg-zinc-900 shadow-xl overflow-hidden">
          {filtered.length === 0 ? (
            ...
            filtered.map((c) => {
              ...
              return (
                ...
                >
                  <span className="font-semibold text-zinc-200 w-11 shrink-0">{c.symbol}</span>
                  <span className="text-zinc-500 flex-1 truncate">{c.name}</span>
                  {livePrice != null ? (
                    <span className="text-emerald-400 shrink-0 font-mono">${formatPrice(livePrice)}</span>
                  ) : (
...
export function HoldingsInput({ holdings, onChange, disabled, walletAddress }: HoldingsInputProps) {
  ...
  const applyPrices = useCallback(
    (p: Record<string, number>, targetHoldings: Holding[]) => {
      setPrices((prev) => ({ ...prev, ...p }));
      if (disabled) return;
      const updated = targetHoldings.map((h) => {
        const sym = h.symbol?.toUpperCase();
        const livePrice = sym ? p[sym] : undefined;
        if (livePrice != null && h.amount > 0) return { ...h, valueUsd: h.amount * livePrice };
        return h;
      });
      const changed = updated.some((h, i) => h.valueUsd !== targetHoldings[i].valueUsd);
  ...
  const fetchPrices = useCallback(
    (syms: string[], targetHoldings: Holding[]) => {
      if (syms.length === 0) return;
      fetch(`/api/prices?symbols=${syms.join(',')}`)
        .then((r) => r.json())
        .then(({ prices: p }: { prices: Record<string, number> }) => {
          if (Object.keys(p).length > 0) applyPrices(p, targetHoldings);
        })
        .catch(() => {});
    },
    [applyPrices]
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      if (symbols.length > 0) fetchPrices(symbols, current);
    }, 30_000);
  ...
  const updateSymbol = (i: number, symbol: string) => {
    const next = [...holdings];
    if (!next[i]) return;
  ...
  return (
    <div className="space-y-2">
      ...
      {walletHasNoHoldings ? (
        ...
        <div className="rounded-md border border-zinc-800 bg-zinc-950 overflow-visible">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-zinc-800 bg-zinc-900/50">
                <th className="text-left px-3 py-2 font-medium text-zinc-500 text-xs w-44">Coin</th>
              ...
              {holdings.map((h, i) => {
                ...
                return (
                  <tr key={i} className="border-t border-zinc-800">
                    <td className="px-3 py-1.5 overflow-visible">
                      <CryptoSelect value={h.symbol} onChange={(s) => updateSymbol(i, s)} disabled={disabled} prices={prices} />
                    </td>
