/**
 * Mudra SDK Audit Module — Hedera "No Solidity Allowed" bounty
 *
...
async function getOrCreateReceiptTopic(): Promise<string> {
  ...
  try {
    ...
    const tx = await new TopicCreateTransaction()
      .setTopicMemo('Mudra SDK-only audit receipts')
      .execute(client);
...
export async function createSdkReceipt(params: {
  ...
  if (params.type === 'EXECUTION' && params.loyaltyPoints && params.loyaltyPoints > 0) {
    ...
    loyalty = {
      ...
      points,
      reason: `Completed Mudra run: session ${params.sessionId}`,
      tokenId,
      htsTxId,
