...
/**
 * Creates the AegisOS Agent iNFT collection on Hedera Testnet (once per env).
...
async function getOrCreateNftCollection(): Promise<string> {
  ...
  try {
    const tx = await new TokenCreateTransaction()
      .setTokenName('AegisOS Agent iNFT')
      ...
      .setTokenMemo('AegisOS on-chain AI agent identity — 0g iNFT')
      .execute(client);
...
/**
 * Mints a single AegisOS agent iNFT on Hedera.
 *
